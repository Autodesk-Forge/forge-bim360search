...
namespace forgeSample.Controllers.AWS
{
  public class Signature
  {
    ...
    public static SortedDictionary<string, string> SignatureHeader(
      ...
      string verb, string content, string absolutePath)
    {
      var hashedRequestPayload = CreateRequestPayload(content);
      ...
      byte[] signingKey = GetSignatureKey(Config.AWSSecret, dateStamp, RegionName.SystemName, ServiceName);
      ...
      string authorization = string.Format("{0} Credential={1}/{2}/{3}/{4}/aws4_request, SignedHeaders={5}, Signature={6}",
        Algorithm, Config.AWSKey, dateStamp, RegionName.SystemName, ServiceName, SignedHeaders, signature);
